...
return declare('code4ge.geo.Map', [ _Widget ], {
	...
	buildRendering: function() {
        ...
        this.container = dojo.create('div', { id: this.id + '_container' }, this.domNode);
		...
		this.surface = gfx.createSurface(this.container, containerBounds.w, containerBounds.h);
		...
		if(typeof this.shapeData == "object") {
			this._init(this.shapeData);
		}
        else {
	        // load map shape file
			if(typeof this.shapeData == "string" && this.shapeData.length > 0) {
				xhr.get({
					url: this.shapeData,
					handleAs: "json",
					sync: true,
			...
			onComplete: function(items) {
				...
				arr.forEach(items, function(item) {
					...
					if(mapInstance.mapObj.features[id]) {
						...
						if(typeof itemVal !== "undefined" && itemVal !== null) {
