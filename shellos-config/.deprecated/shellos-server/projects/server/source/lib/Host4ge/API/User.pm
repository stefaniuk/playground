...
sub _chcek_by_id {
    ...
    my $min_gid = $self->{conf}->get('user_min_id');
    my $max_gid = $self->{conf}->get('user_max_id');
...
sub create {
    ...
    if(!$self->exists($user)) {
        ...
        if($home !~ m/^$hosting_dir\/(accounts|applications)\/[a-zA-Z0-9\/._-]+$/) {
        ...
        system("useradd -K UID_MIN=$min_uid -K UID_MAX=$max_uid -K GID_MIN=$min_gid -K GID_MAX=$max_gid -d $home -s $shell $password -g $group $groups $user > /dev/null 2>&1");
        if($? != 0) {
            handle_error($?);
            return FALSE;
        }
        ...
        dir_create("$home/dev", 'root', 'root', '0755');
        dir_create("$home/etc/pam.d", 'root', 'root', '0755');
